	...
	return (
		<div className="space-y-4">
			<div>
				...
				<p className="text-muted-foreground text-sm">Select a Bitcoin allocation strategy that matches your risk tolerance and investment goals.</p>
			</div>
						...
						<Card
							...
							<div className="space-y-3">
								...
								<div className="space-y-2">
									<div className="text-xs">
										<div className="flex justify-between">
											...
											<span className="font-medium text-orange-600">{strategy.bitcoinAllocation}%</span>
										</div>
										<div className="flex justify-between">
											<span className="text-muted-foreground">Traditional:</span>
											<span className="font-medium text-blue-600">{strategy.traditionalAllocation}%</span>
										</div>
									</div>
									...
									<div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
										<div className="h-full flex">
											{strategy.bitcoinAllocation > 0 && (
												<div className="bg-orange-500" style={{ width: `${strategy.bitcoinAllocation}%` }} />
											)}
											{strategy.traditionalAllocation > 0 && (
												<div className="bg-blue-500" style={{ width: `${strategy.traditionalAllocation}%` }} />
											)}
										</div>
									</div>
								</div>
								...
								<div className="flex justify-center">
									<Badge variant="outline" className={`text-xs ${getRiskColor(strategy.riskProfile)}`}>
			...
			{selectedStrategy && (
				<div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
					<div className="flex items-center gap-2 mb-2">
						...
						<span className="font-medium text-blue-900">Selected: {STRATEGY_CONFIGS[selectedStrategy].name}</span>
					</div>
					<p className="text-sm text-blue-800">
						{STRATEGY_CONFIGS[selectedStrategy].description} - This strategy allocates{" "}
						<strong>{STRATEGY_CONFIGS[selectedStrategy].bitcoinAllocation}% to Bitcoin</strong> and{" "}
						<strong>{STRATEGY_CONFIGS[selectedStrategy].traditionalAllocation}% to traditional assets</strong>.
					</p>
				</div>
			)}
		</div>
